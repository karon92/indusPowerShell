...
## 04/03/2019   Laurent DEVOUCOUX       Création de la fonction getListFreeDrive
##
## VERSION 0.0.3 
##
...
Function getListFreeDrive
{
    ...
    foreach ($d in $driveLetters) {
        if(!(Test-Path -Path "$d`:" -IsValid)) {
            #return $d
            $ListFreeDrive += ,@("$d")
            #Write-Host ("ListFreeDrive : {0}" -f $ListFreeDrive[0])
        }
    }
    return $ListFreeDrive
}